...
const Searchbar = ({ searchTerm, setSearchTerm, accessToken, setAccessToken, fetchAlbums, setAlbums, fetchTracks, setTracks}) => {
  ...
  async function fetchResults() {
    ...
    var trackArtistId = await fetch("https://api.spotify.com/v1/search?q=" + searchTerm + "&type=track,artist", artistParams)
    .then((res) => res.json())
    ...
    // .then(console.log("Artists ", fetchAlbums, "tracks ", fetchTracks))


    // // get artist id
    // var artistId = await fetch(
    //   "https://api.spotify.com/v1/search?q=" + searchTerm + "&type=artist&include_external=audio",
    //   artistParams
    // )
    //   .then((res) => res.json())
    //   .then((data) => {
